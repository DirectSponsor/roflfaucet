...
// Game State
    ...
    async animateCreditsIncrease(totalPayout) {
        ...
        const currentBalanceBackElement = document.getElementById('current-balance-back');
        ...
        // Animate the credits counter on both front and back sides
        if (currentBalanceElement) {
            this.animateCounter(currentBalanceElement, startCredits, endCredits, 800);
        }
        if (currentBalanceBackElement) {
            this.animateCounter(currentBalanceBackElement, startCredits, endCredits, 800);
        }
    }
    ...
    async loadGameState() {
        ...
        // Enable spin button now that balance is loaded
    ...
    processWin(winResult, bet) {
        ...
        if (totalPayout > 0) {
            ...
            // No win - update display to show 0
            const lastWinElement = document.getElementById('last-win');
            if (lastWinElement) {
    ...
    updateDisplay() {
        // Update all display elements (both front and back sides)
        const currentBalanceElement = document.getElementById('current-balance');
        const currentBalanceBackElement = document.getElementById('current-balance-back');
        const currentBetElement = document.getElementById('current-bet');
        const currentBetBackElement = document.getElementById('current-bet-back');
        const lastWinBackElement = document.getElementById('last-win-back');
        
        if (currentBalanceElement) currentBalanceElement.textContent = Math.floor(this.credits);
        if (currentBalanceBackElement) currentBalanceBackElement.textContent = Math.floor(this.credits);
        if (currentBetElement) currentBetElement.textContent = this.currentBet;
        if (currentBetBackElement) currentBetBackElement.textContent = this.currentBet;
        
        // Also update the last win on back side to match front
        const lastWinElement = document.getElementById('last-win');
        if (lastWinElement && lastWinBackElement) {
            lastWinBackElement.textContent = lastWinElement.textContent;
        }
    ...
    enableSpinButton() {
        // Enable the spin button now that balance is loaded
        const spinButton = document.getElementById('spin-btn');
        if (spinButton) {
            spinButton.disabled = false;
            console.log('🎰 Spin button enabled - balance loaded:', this.credits);
        }
        
        // Also enable bet buttons
    ...
    loadLastBet() {
        ...
        const savedBet = localStorage.getItem('slotsLastBet');
    ...
    saveLastBet() {
        ...
        localStorage.setItem('slotsLastBet', this.currentBet.toString());
...
function decreaseBet() {
    if (slotMachine && !slotMachine.isSpinning) {
        if (slotMachine.currentBet > 1) {
            ...
            slotMachine.saveLastBet(); // Save the new bet amount
