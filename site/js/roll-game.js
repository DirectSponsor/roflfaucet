...
class RollGame {
    ...
    init() {
        console.log('🎲 Initializing Roll Game - VERSION 2.2 (NO MORE CONSTANT POLLING - Event Driven Balance)...');
        console.log('🚀 BALANCE OPTIMIZATION ACTIVE: No more setInterval polling!');
        console.log('📊 Balance will only update on game actions, not every 5 seconds');
        ...
        const initBalance = () => {
            if (typeof UnifiedBalanceSystem !== 'undefined') {
                ...
                this.loadLastBet(); // Load saved bet amount
                this.loadGameStats();
                this.updateDisplay();
    ...
    setupEventDrivenBalance() {
        console.log('🔄 Setting up event-driven balance system for Roll Game (no constant polling)');
        ...
        // Update balance on payout changes (less frequent sync)
        const originalSetPayout = this.setPayout.bind(this);
        this.setPayout = async (amount) => {
            originalSetPayout(amount);
            // Skip balance refresh for payout changes (optimization)
        };
    }
    ...
    async smartBalanceRefresh(trigger) {
        ...
        try {
            console.log(`🔄 Roll Game: Smart balance refresh triggered: ${trigger}`);
            // Refresh balance for important triggers
            if (trigger === 'before_game' || trigger === 'after_game') {
                await this.refreshBalance();
            }
        } catch (error) {
            console.warn('Roll Game: Smart balance refresh failed:', error);
    ...
    loadLastBet() {
        ...
        const savedBet = localStorage.getItem('diceLastBet');
        const lastBet = savedBet ? parseInt(savedBet) : 1;
        const betInput = document.getElementById('betAmount');
        if (betInput) {
            betInput.value = Math.max(1, lastBet);
    ...
    saveLastBet(bet) {
        ...
        localStorage.setItem('diceLastBet', bet.toString());
        console.log(`🎲 Saved bet amount: ${bet}`);
    }
    ...
    setBet(amount) {
        ...
        this.saveLastBet(bet); // Save bet amount to localStorage
        this.updateProbability();
    }
    ...
    toggleHighLow() {
        ...
        if (this.isLowMode) {
            toggle.textContent = 'Switch to 🔼';
            condition.textContent = 'Roll 🔽';
        } else {
            toggle.textContent = 'Switch to 🔽';
            condition.textContent = 'Roll 🔼';
        }
    ...
    async roll() {
        ...
        try {
            // IMMEDIATE balance deduction (like slots and wheel games)
            console.log("🎲 Deducting balance immediately...");
    ...
    animateRoll() {
        ...
        // Clear previous result before starting new roll
        resultContainer.className = 'dice-result-section rolling';
        rollStatus.textContent = 'Rolling...';
        rollPayout.textContent = ''; // Clear previous payout display
    ...
    processRoll(betAmount, payoutMultiplier) {
        ...
        if (isWin && winAmount > 0) {
            // Use await and get the returned balance for immediate update
            this.balanceSystem.addBalance(winAmount, 'dice_win').then(result => {
                if (result && result.balance !== undefined) {
                    // Update balance display immediately with the returned balance
                    this.updateBalanceDisplay(result.balance);
                    console.log(`🎲 Balance updated immediately after win: ${result.balance}`);
                }
            }).catch(error => {
                console.error('Error updating balance after dice win:', error);
                // Fallback to refresh balance
                this.refreshBalance();
            });
        }
    ...
    showRollResult(rollResult, isWin, winAmount, betAmount) {
        ...
        // Set win/lose status
        if (isWin) {
            resultContainer.className = 'dice-result-section win';
            rollStatus.textContent = 'YOU WIN! 🎉';
            rollPayout.textContent = `+${winAmount} ${currency}`;
            this.playSound('win');
        } else {
            resultContainer.className = 'dice-result-section lose';
            rollStatus.textContent = 'You lose 😢';
            rollPayout.textContent = `-${betAmount} ${currency}`;
            this.playSound('lose');
        }
        
        // Result will stay until next roll starts
        // No auto-clear timeout anymore
    }
    ...
    updateStatsDisplay() {
        ...
        // Only update elements if they exist (optional stats display)
        ...
        if (totalRollsEl) totalRollsEl.textContent = this.gameStats.totalRolls.toLocaleString();
    ...
    updateRecentRollsDisplay() {
        ...
        if (this.recentRolls.length === 0) {
            container.innerHTML = `
                <div style="text-align: center; padding: 2rem; color: #666;">
                    ...
                    <div>No recent rolls yet!</div>
                    <div style="font-size: 0.9rem; margin-top: 0.5rem;">Start playing to see your roll history here.</div>
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = this.recentRolls.map(roll => {
            ...
            const currency = this.balanceSystem && this.balanceSystem.isLoggedIn ? 'coins' : 'tokens';
            const resultAmount = roll.isWin ? `+${roll.winAmount} ${currency}` : `-${roll.betAmount} ${currency}`;
            
            return `
                <div class="roll-entry ${resultClass}">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div><strong>Roll:</strong> ${roll.rollValue}</div>
    ...
    saveRecentRolls() {
        try {
            localStorage.setItem('diceRecentRolls', JSON.stringify(this.recentRolls));
        } catch (error) {
            console.error('Error saving recent rolls:', error);
        }
    }
    ...
    loadGameStats() {
        try {
            ...
            const savedRolls = localStorage.getItem('diceRecentRolls');
            if (savedRolls) {
                this.recentRolls = JSON.parse(savedRolls);
                console.log(`🎲 Loaded ${this.recentRolls.length} recent rolls from localStorage`);
            }
        } catch (error) {
