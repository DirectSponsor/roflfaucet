...
class LevelsSystem {
    ...
    async loadUserLevel() {
        ...
        try {
            ...
            if (this.isLoggedIn) {
                ...
                console.log(`🔎 [DEBUG] Combined User ID from sessionStorage: ${combinedUserId}`);
    ...
    async saveUserLevel() {
        ...
        try {
            ...
            if (this.isLoggedIn) {
                ...
                const combinedUserId = localStorage.getItem('combined_user_id');
                console.log(`🔎 [DEBUG] Saving for combined user ID: ${combinedUserId}`);
    ...
    async purchaseUpgrade() {
        ...
        try {
            ...
            const userId = localStorage.getItem('combined_user_id');
            const response = await fetch('api/simple-profile.php?action=update_level', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
        ...
        if (nextLevel) {
            ...
            content += `
                ...
                <div style="display: flex; gap: 10px; justify-content: center;">
                    <button onclick="document.getElementById('insufficient-level-modal').remove()" 
                            style="background: #6c757d; color: white; border: none; padding: 12px 20px; border-radius: 5px; cursor: pointer;">Close</button>
                    <button onclick="window.location.href='https://auth.directsponsor.org/jwt-login.php?redirect_uri=' + encodeURIComponent(window.location.origin + window.location.pathname)" 
