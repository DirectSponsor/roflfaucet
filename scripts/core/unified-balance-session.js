// Unified Balance System - Session-Based Architecture
// Works the same for both guest users (localStorage) and members (PHP sessions)
...
class UnifiedBalanceSystem {
    constructor() {
        ...
        console.log(`💰 Unified Balance System (Session-Based) initialized for ${this.isLoggedIn ? 'member' : 'guest'} user`);
    }
    ...
    async getRealBalance() {
        try {
            // Use our new session-based API instead of old data server
            const response = await fetch('/local-faucet-api.php?action=balance', {
                method: 'GET',
                headers: {
            ...
            if (response.ok) {
                ...
                this.balance = Math.floor(parseFloat(data.balance) || 0);
                console.log('✅ Real balance loaded from session:', this.balance);
                return this.balance;
            } else {
                console.log('⚠️ Session API unavailable, using fallback balance');
                // Don't reset login status - just use a fallback balance
                this.balance = 0;
                return this.balance;
            }
        } catch (error) {
            console.error('💥 Session balance loading error:', error);
            // Don't reset login status - just use a fallback balance
            this.balance = 0;
            return this.balance;
        }
    ...
    async addRealBalance(amount, source, description) {
        try {
            // Use our new session-based API for instant balance updates
            const response = await fetch('/local-faucet-api.php', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${this.accessToken}`,
            ...
            if (response.ok) {
                const data = await response.json();
                if (data.success) {
                    this.balance = Math.floor(data.new_balance);
                    console.log('✅ Real balance added via session:', amount, 'New balance:', this.balance);
                    return { success: true, balance: this.balance };
                } else {
                    console.log('⚠️ Session API failed, falling back to guest mode');
                    return this.addGuestBalance(amount, source, description);
                }
            } else {
                console.log('⚠️ Session API unavailable, falling back to guest mode');
                return this.addGuestBalance(amount, source, description);
            }
        } catch (error) {
    ...
    async subtractRealBalance(amount, source, description) {
        try {
            // For subtracting, we could add a new endpoint or use the existing API
            // For now, let's handle this locally and sync later
            const currentBalance = await this.getRealBalance();
            
            if (currentBalance < amount) {
                console.log('❌ Insufficient real balance');
                return { success: false, balance: currentBalance, error: 'Insufficient balance' };
            }
            
            // This is a placeholder - we'd need to add a subtract endpoint to local-faucet-api.php
            // For now, fall back to guest behavior for consistency
            console.log('⚠️ Real balance subtraction not implemented yet, using fallback');
            return this.subtractGuestBalance(amount, source, description);
    ...
    getTerminology() {
        return {
            ...
            fullName: this.isLoggedIn ? 'UselessCoins' : 'PointlessTokens',
...
window.updateBalanceDisplays = async () => {
    ...
    console.log('💰 Balance displays updated (session-based):', Math.floor(balance), terminology.currency);
};
...
window.updateCurrencyDisplays = () => {
    ...
    currencyFullElements.forEach(element => {
        element.textContent = terminology.currency;  // Changed from fullName to currency for shorter display
        element.title = terminology.fullName;
    });
    
    console.log('🏷️ Currency displays updated (session-based):', terminology.currency);
};
...
console.log('🔧 Unified Balance System (Session-Based) ready!');
